...
import type { Chat, Images } from "@/@types/chat";
...
const ChatDetail = ({ id, info }: { id: string; info: any }) => {
  const { setIsMeeting } = use(AppContext);
  ...
  useEffect(() => {
    if (socketRef.current) {
      ...
      socketRef.current.on("join_channel", (data: any) => {
        console.log(data);
        if (data.meeting) setIsMeeting(data.meeting.url);
      });
      socketRef.current.on("message", (dataChat: any) => {
        ...
        const dataByDate =
          data &&
          data.filter(
            (item: { date: string; data: Chat[] }) => item.date === currentDate,
          );
        if (dataByDate.length === 0) {
          setData((prev: any) => [
            ...prev,
            { date: currentDate, data: [dataChat] },
          ]);
          return;
        }
        setData((prev: any) =>
          prev.map((item: { date: string; data: Chat[] }) =>
            item.date === currentDate
              ? { ...item, data: [...item.data, dataChat] }
              : item,
          ),
        );
        scrollToBottom();
      });
      socketRef.current.on("on_meeting", (dataMeet: any) => {
        setIsMeeting(dataMeet.url);
      });
      socketRef.current.on("off_meeting", () => {
        setIsMeeting(null);
      });
      return () => {
  ...
  const handleScrollToReply = async (id: string) => {
    ...
    if (includeCurrentChat.length > 0) {
      ...
      while (!isIncludeData && unread > 0) {
        ...
        skip = read;
        isIncludeData =
          res.data.data.filter((d: Chat) => d._id === id).length > 0;
      }
      if (isIncludeData) {
  ...
  return (
    <div className="col-span-5 relative h-[98%] flex py-2 overflow-hidden">
      <div
        className={`relative ${isRightPanel ? "w-4/5" : "w-full"} h-full flex flex-col transition-all duration-500`}
      >
        <ChatHeader
          info={info}
      ...
      <div
        className={`${isRightPanel ? "w-1/5 px-1" : "w-0 px-0 opacity-0"} h-full transition-all duration-500`}
      >
        <ChatRightPanel
          info={info}
