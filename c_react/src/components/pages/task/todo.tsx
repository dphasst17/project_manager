...
const {statusTask,setStatusTask} = useTaskStore()
  ...
  const handleChangeStatus = async (id:number,current:string) => {
    ...
    const result = statusTask?.in_progress.data.filter((item:any) => item.task_id === id)
    statusTask && setStatusTask({
      ...statusTask,
      completed: {
        ...statusTask.completed,
        data: [...statusTask.completed.data,...result!]
      },
      in_progress: {
        ...statusTask?.in_progress,
        data: statusTask?.in_progress.data.filter((item:any) => item.task_id !== id)
      }
    })
  }
  return statusTask && <div className="todo-task w-full min-h-[200px] flex flex-col items-center">
    <h1 className="text-2xl font-semibold">To Do</h1>
    <div className="w-4/5 grid grid-cols-4 gap-2">
      {statusTask?.in_progress.data.map((item:any) => 
