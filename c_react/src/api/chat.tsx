...
const adminGetAll = async(token:string,limit:number,skip:number) => {
  const res = await fetch(`${import.meta.env.VITE_URL_SERVER}/chat/admin?skip=${skip}&limit=${limit}`,{
    method: 'GET',
    headers: {
...
const getByEmployee = async(token:string,limit:number,skip:number) => {
  const res = await fetch(`${import.meta.env.VITE_URL_SERVER}/chat/employee?skip=${skip}&limit=${limit}`,{
    method: 'GET',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${token}`
...
const getChatByChannelId = async(id:string,limit:number,skip:number) => {
  const res = await fetch(`${import.meta.env.VITE_URL_SERVER}/chat/detail/${id}?skip=${skip}&limit=${limit}`,{
    method: 'GET',
  })
  return res.json()
}
const getImagesByChannelId = async(id:string,limit:number,skip:number) => {
  const res = await fetch(`${import.meta.env.VITE_URL_SERVER}/chat/images/${id}?skip=${skip}&limit=${limit}`,{
    method: 'GET',
  })
  return res.json()
}
...
const createChannel = async(token:string,data: any) => {
  const res = await fetch(`${import.meta.env.VITE_URL_SERVER}/chat/channel`,{
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': `Bearer ${token}`
