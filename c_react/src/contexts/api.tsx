...
export const ApiProvider = ({ children }: { children: React.ReactNode }) => {
    ...
    const {isLog,isAdmin,setChannel} = use(AppContext);
    ...
    const mutationEmployee = useMutation({
      mutationFn: async (token:string) => {
        const [project,completed_task,in_progress_task,not_started_task,overdue_task,review_task,chat] = await Promise.all([
          ...
          getTaskByStatus('employee',token,'overdue',1,12),
          getTaskByStatus('employee',token,'review',1,12),
          getByEmployee(token,20,0)
        ])
        return {project,completed_task,in_progress_task,not_started_task,overdue_task,review_task,chat}
      },
      onSuccess: (data:any) => {
        const {project,completed_task,in_progress_task,not_started_task,overdue_task,review_task,chat} = data
        ...
        && overdue_task.status === 200 && review_task.status === 200
        && setStatusTask({completed:completed_task.data,in_progress:in_progress_task.data,not_started:not_started_task.data,overdue:overdue_task.data,review:review_task.data})
      },
      onError: (error:any) => {
        console.log(error)
      }
    ...
    useEffect(() => {
      ...
      const getData = async () => {
          const token = await getToken();
          if (token) {
            isAdmin ? mutationAdmin.mutate(token) : mutationEmployee.mutate(token)
          }
          setIsPending(false)
        }
        getData()
