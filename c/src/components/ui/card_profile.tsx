...
const CardProfile = ({name,role,email}:{name:string,role:string,email:string}) => {
  return (
    <div className="flex items-center p-3 w-72 h-28 bg-white rounded-md shadow-lg">
      ...
      <section className="block border-l border-gray-300 m-3">
        <div className="pl-3">
          <h3 className="text-gray-600 text-lg font-bold">{role}</h3>
          <h3 className="text-gray-600 font-semibold text-sm">{name}</h3>
        </div>
        <div className="flex gap-3 pt-2 pl-3 text-gray-600">
          {email}
        </div>
      </section>
    </div>
